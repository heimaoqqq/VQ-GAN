...
class Unet(nn.Module):
    ...
    def __init__(self, im_channels, model_config):
        ...
        self.class_embed_dim = None  # 添加类别嵌入维度
        ...
        if self.condition_config is not None:
            ...
            if 'class' in condition_types:
                ...
                self.class_embed_dim = self.t_emb_dim  # 使用时间嵌入维度作为类别嵌入维度
        ...
        # 为类别条件添加投影层，用于生成交叉注意力的上下文
        if self.class_cond:
            self.class_context_proj = nn.Sequential(
            nn.Linear(self.t_emb_dim, self.t_emb_dim),
            nn.SiLU(),
            nn.Linear(self.t_emb_dim, self.t_emb_dim)
        )
        ...
        if self.text_cond:
            ...
            context_dim = self.class_embed_dim
        
        # 启用交叉注意力的条件
        use_cross_attn = self.text_cond or self.class_cond
    ...
    def forward(self, x, t, cond_input=None):
        ...
        # 处理类别条件 - 为交叉注意力创建上下文
            ...
            # 将类别嵌入添加到时间嵌入中 - 保持原有行为
            t_emb += class_embed
            
            # 为交叉注意力创建上下文
            # 将类别嵌入转换为序列形式 [batch_size, 1, embed_dim]
            batch_size = x.shape[0]
            class_context = class_embed.unsqueeze(1)  # [B, 1, D]
            
            # 投影到适当的维度
            class_context = self.class_context_proj(class_context)
            
            # 设置为交叉注意力的上下文
            context_hidden_states = class_context
        ####################################
